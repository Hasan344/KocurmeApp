-- ============================================================
-- Zal köçürmə analizi — SEÇİLMİŞ İMTAHAN üçün
--
-- @ExamId: çıxarış ediləcək konkret imtahan.
--
-- Nəticədə YALNIZ həmin imtahanın zal sətirləri qaytarılır,
-- LAKİN normallaşdırma (Kolon3/4/5) və "Orta qiymət" sətri
-- həmin imtahanın SİNFİNƏ aid BÜTÜN imtahanların QLOBAL ortasına
-- görə hesablanır (docx-dəki məntiq).
--
--   Detal sətir (RowType = 0):
--       Kolon3 = Faiz1 / QlobalAvgFaiz1   (məs. 2.78/4.72 = 0.59)
--       Kolon4 = Faiz2 / QlobalAvgFaiz2
--   "Orta qiymət" sətri (RowType = 1):
--       Faiz1  = bu imtahanın orta Faiz1-i
--       Kolon3 = buImtahanOrtaFaiz1 / QlobalAvgFaiz1  (məs. 3.54/4.72 = 0.75)
--
-- Qlobal orta = Exams.Sinif = (bu imtahanın Sinif-i) olan bütün imtahanlar üzrə.
--
-- İstifadə: SELECT * FROM dbo.fn_CheatingRoomStatsForExam(5);
-- ============================================================

-- Exams cədvəlinə Sinif (imtahan tipi: 9 / 11) sütunu (idempotent)
IF COL_LENGTH('dbo.Exams', 'Sinif') IS NULL
    ALTER TABLE dbo.Exams ADD Sinif INT NULL;
GO

CREATE OR ALTER FUNCTION dbo.fn_CheatingRoomStatsForExam (@ExamId INT)
RETURNS @res TABLE
(
    ExamId       INT,
    Zal          SMALLINT,
    ...
    Kolon4       DECIMAL(5,2),
    Kolon5       DECIMAL(5,2),
    RowType      TINYINT          -- 0 = detal, 1 = "Orta qiymət" sətri
)
AS
BEGIN
    -- Seçilmiş imtahanın sinfi
    DECLARE @Sinif INT = (SELECT Sinif FROM dbo.Exams WHERE Id = @ExamId);

    -- Hər (imtahan, zal) üzrə Faiz1 / Faiz2 — QLOBAL dəst (bu sinfin bütün imtahanları)
    DECLARE @faiz TABLE
    (
        ExamId       INT,
        ZAL1         SMALLINT,
    ...
    SELECT
        ...
        s.FennSayi,
        s.OdaSayi,
        CAST((CAST(s.FennSayi    AS DECIMAL(10,2)) / NULLIF(s.OdaSayi * 2, 0)) * 100 AS DECIMAL(5,2)),
    ...
    FROM (
        ...
        WHERE m1.EYNI_Y >= 5
          AND m1.ExamId IN (
                ...
                WHERE (@Sinif IS NOT NULL AND Sinif = @Sinif)
                   OR (@Sinif IS NULL AND Id = @ExamId)
              )
    ...
    -- Detal sətirlər — YALNIZ seçilmiş imtahan (qlobal normallaşdırma ilə)
    ...
    -- "Orta qiymət" sətri — seçilmiş imtahanın ortası / qlobal orta
    INSERT INTO @res (ExamId, Zal, KocurenSayi, FennSayi, OdaSayi, Faiz1, Faiz2, Kolon3, Kolon4, Kolon5, RowType)
    SELECT
        @ExamId,
        CAST(0 AS SMALLINT),
        ...
        1
    FROM (
        SELECT AVG(Faiz1) AS AvgF1, AVG(Faiz2) AS AvgF2
        FROM @faiz
        WHERE ExamId = @ExamId
    ) e
    WHERE e.AvgF1 IS NOT NULL;
